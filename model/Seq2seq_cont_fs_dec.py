...
class Dialog_Seq2seq_cont(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        if "en_encoder_input_cc" in batch and batch["en_encoder_input_cc"] != None:
            ...
            # Decoder side
            proj_en_h = self.projection2(encoder_outputs_en_cc)
            proj_fo_h = self.projection2(encoder_outputs_fo_cc)
            avg_cc_en = self.avg_pool(proj_en_h, en_attention_mask_cc) # (bt, h)
            avg_cc_fo = self.avg_pool(proj_fo_h, fo_attention_mask_cc) # (bt, h)
            
            
            # English
            if "encoder_input_gwoz" in batch and batch["encoder_input_gwoz"] != None:
                
                proj_dg_h = self.projection2(sequence_output_en)
                avg_dg_en = self.avg_pool(proj_dg_h, decoder_attention_mask_en) # (bd, h)
                
                new_vec = torch.cat([avg_cc_en[0, :].unsqueeze(0), avg_cc_fo[1:, :]], dim=0) # Positive sample is the first.
                
                cos = nn.CosineSimilarity(dim=-1)
                sim_matrix = cos(avg_dg_en.unsqueeze(1), new_vec.unsqueeze(0)) # (bd, bt)
                logits = sim_matrix / self.tau
                batch_size = sequence_output_en.size(0)
                labels = torch.zeros(batch_size, device=sequence_output_en.device).long()
                cont2_loss_en = self.cont_crit(logits, labels)
                cont_loss += cont2_loss_en
                
            # Foreign
            if "encoder_input_gwoz_fs" in batch and batch["encoder_input_gwoz_fs"] != None:
                
                proj_dg_h = self.projection2(sequence_output_fo)
                avg_dg_fo = self.avg_pool(proj_dg_h, decoder_attention_mask_fo) # (bd, h)
                
                new_vec = torch.cat([avg_cc_fo[0, :].unsqueeze(0), avg_cc_en[1:, :]], dim=0) # Positive sample is the first.
                
                cos = nn.CosineSimilarity(dim=-1)
                sim_matrix = cos(avg_dg_fo.unsqueeze(1), new_vec.unsqueeze(0)) # (bd, bt)
                logits = sim_matrix / self.tau
                batch_size = sequence_output_fo.size(0)
                labels = torch.zeros(batch_size, device=sequence_output_fo.device).long()
                cont2_loss_fo = self.cont_crit(logits, labels)
                cont_loss += cont2_loss_fo
