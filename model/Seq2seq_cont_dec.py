...
class Dialog_Seq2seq_cont(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        nll_fo, cont_loss, cont2_loss = 0, 0, 0
        ...
        if "encoder_input_gwoz" in batch and batch["encoder_input_gwoz"] != None:
            ...
            input_ids_fo=batch["encoder_input_gwoz_trans"]
            attention_mask_fo=batch["attention_mask_gwoz_trans"]
            labels_fo=batch["decoder_output_gwoz_trans"]
            
            # Get encoder outputs
            encoder = self.model.get_encoder()
            decoder = self.model.get_decoder()
            encoder_outputs_en = encoder(input_ids=input_ids,
                                        attention_mask=attention_mask,
                                        )[0]

            encoder_outputs_fo = encoder(input_ids=input_ids_fo,
                                        attention_mask=attention_mask_fo,
                                        )[0]
            # Get fo logits
            decoder_input_ids = self._shift_right(labels_fo)
            ...
            decoder_outputs = decoder(
                ...
                encoder_attention_mask=attention_mask_fo
            )
            ...
            nll_fo = self.criterion(lm_logits.view(-1, vocab_size), labels_fo.view(-1))
        ...
        if "en_encoder_input_cc" in batch and batch["en_encoder_input_cc"] != None:
            ...
            # Contrastive Learning on encoder side
            ...
            avg_dg = self.avg_pool(proj_dg_h, decoder_attention_mask) # (bd, h)
        ...
        #     input_ids=decoder_input_ids,
        #     attention_mask=decoder_attention_mask,
        #     encoder_hidden_states=encoder_outputs_en,
        ...
        # lm_logits = self.model.lm_head(sequence_output)
        # vocab_size = lm_logits.size(-1)
        # nll_en = self.criterion(lm_logits.view(-1, vocab_size), labels.view(-1))
        
        return nll_fo + cont_loss + cont2_loss
